"""
Rules for generating coverage tracks for single-end samples using bamCoverage from deepTools
...
# Rule for generating coverage track with all reads for single-end samples
rule generate_all_reads_coverage_single:
    ...
    shell:
        ...
        # Log start
        echo "Generating coverage track (all reads) for single-end sample {wildcards.run_tag}" > {log}
...
# Rule for generating coverage track with uniquely mapped reads only for single-end samples
rule generate_unique_reads_coverage_single:
    wildcard_constraints:
        run_tag = "|".join([re.escape(rt) for rt in SINGLE_RUN_TAGS]) if SINGLE_RUN_TAGS else "^$"
    input:
        bam = get_picard_bam,
    ...
    output:
        ...
        flag = get_processing_path("{run_tag}/coverage_single_complete.flag")
    log:
    ...
    shell:
        ...
        # Log start
        echo "Generating coverage track (unique reads) for single-end sample {wildcards.run_tag}" > {log}
        echo "Input BAM: {input.bam}" >> {log}
        echo "Output: {output.bigwig}" >> {log}
        echo "Bin size: {params.bin_size}" >> {log}
        echo "Normalization: {params.normalize}" >> {log}
        ...
        echo "Coverage track generation completed for single-end sample {wildcards.run_tag}" > {output.flag}
