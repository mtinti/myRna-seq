"""
Rules for alignment with bowtie2 for single-end reads
Including read group information for Picard compatibility
"""
import os
import re

# Rule for aligning single-end reads with bowtie2
rule align_single_end:
    ...
    params:
        ...
        rg_pl = "ILLUMINA"
    log:
        get_processing_path("{sample}/logs/alignment_single.log")
    benchmark:
        get_processing_path("{sample}/benchmarks/align_single.benchmark.txt")
    threads: 
        config["cores_align"]
    resources:
        mem_mb = 8000
    ...
    shell:
        ...
        # Start logging
        echo "Aligning single-end reads for {wildcards.sample}" > {log}
        ...
        if [[ ! -s {input.r} ]]; then
            echo "ERROR: Input file {input.r} does not exist or is empty!" >> {log}
            exit 1
        fi
        ...
        echo "Alignment complete for single-end sample {wildcards.sample}" > {output.flag}
